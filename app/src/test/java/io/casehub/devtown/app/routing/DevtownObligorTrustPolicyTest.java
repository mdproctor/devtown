...
class DevtownObligorTrustPolicyTest {
    ...
    /**
     * Stub TrustScoreSource — none of its methods should be called because the test
     * overrides {@code currentScore()} in the {@link TrustGateService} subclass.
     */
    private static final TrustScoreSource STUB_SOURCE = new TrustScoreSource() {
        @Override public OptionalDouble globalScore(String actorId) { throw new UnsupportedOperationException(); }
        @Override public OptionalDouble capabilityScore(String actorId, String capability) { throw new UnsupportedOperationException(); }
        @Override public OptionalDouble dimensionScore(String actorId, String dimension) { throw new UnsupportedOperationException(); }
        @Override public OptionalDouble capabilityDimensionScore(String actorId, String capability, String dimension) { throw new UnsupportedOperationException(); }
        @Override public int decisionCount(String actorId, String capability) { throw new UnsupportedOperationException(); }
        @Override public Map<String, Double> allCapabilityScores(String actorId) { throw new UnsupportedOperationException(); }
        @Override public Map<String, Double> allDimensionScores(String actorId) { throw new UnsupportedOperationException(); }
        @Override public Map<String, Double> qualityScores(String actorId, String capability) { throw new UnsupportedOperationException(); }
    };

    /** Creates a TrustGateService subclass that overrides currentScore() to return a fixed value. */
    private static TrustGateService fixedScoreService(OptionalDouble score) {
        return new TrustGateService(STUB_SOURCE) {
            @Override
            public OptionalDouble currentScore(String actorId) {
                return score;
            }
        };
    }
    ...
    void permitsWhenThresholdPrefNull_nullGuardPath() {
        var policy = new DevtownObligorTrustPolicy(EMPTY_PROVIDER, fixedScoreService(OptionalDouble.of(0.05)));
    ...
    void permitsWhenThresholdExplicitZero_explicitDisabledPath() {
        var policy = new DevtownObligorTrustPolicy(GATE_DISABLED, fixedScoreService(OptionalDouble.of(0.05)));
        assertThat(policy.permits(ctx())).isTrue();
    }
    ...
    void permitsBootstrapAgent_noLedgerObservations() {
        var policy = new DevtownObligorTrustPolicy(GATE_030, fixedScoreService(OptionalDouble.empty()));
        assertThat(policy.permits(ctx())).isTrue();
    }
    ...
    void permitsWhenScoreAboveFloor() {
        var policy = new DevtownObligorTrustPolicy(GATE_030, fixedScoreService(OptionalDouble.of(0.50)));
        assertThat(policy.permits(ctx())).isTrue();
    }
    ...
    void permitsWhenScoreAtFloor() {
        var policy = new DevtownObligorTrustPolicy(GATE_030, fixedScoreService(OptionalDouble.of(0.30)));
        assertThat(policy.permits(ctx())).isTrue();
    }
    ...
    void deniesWhenScoreBelowFloor() {
        var policy = new DevtownObligorTrustPolicy(GATE_030, fixedScoreService(OptionalDouble.of(0.20)));
